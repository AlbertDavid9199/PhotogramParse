...
Parse.Cloud.define('GetReportedUsers', function (request, response) {
	...
	new Parse.Query('Report').include('reportedBy').include('reportedUser').include('profile').doesNotExist('actionTaken').descending('createdAt').find().then(function (result) {
...
Parse.Cloud.define('GetReportedUserDetails', function (request, response) {
	...
	var reportsQuery = new Parse.Query('Report').descending('updatedAt').limit(1000).equalTo('reportedBy', reportedBy).find();

	// Load the recent messages the reported user has sent
	var messagesQuery = new Parse.Query('ChatMessage').equalTo('sender', reportedUser).limit(50).descending('createdAt').find();
...
Parse.Cloud.define('BanUser', function (request, response) {
	...
	if (!bannedUserId) response.error('Invalid params');

	var bannedUser = new Parse.User();
	bannedUser.id = bannedUserId;

	var bannedUserQuery = new Parse.Query(Parse.User).include('profile').get(bannedUserId);

	// Close out all reports against this user
	var reportsQuery = new Parse.Query('Report').equalTo('reportedUser', bannedUser).limit(1000).find();
	...
	var matches1Query = new Parse.Query('Match').equalTo('uid1', bannedUserId).equalTo('state', 'M').limit(1000).find();
	var matches2Query = new Parse.Query('Match').equalTo('uid2', bannedUserId).equalTo('state', 'M').limit(1000).find();
...
Parse.Cloud.define('CloseReport', function (request, response) {
	...
	if (!reportId || !action && action !== '') response.error('Invalid parameters ' + JSON.stringify(request.params));
...
Parse.Cloud.define('SearchUsersByEmail', function (request, response) {
	...
	if (!email) response.error('Invalid parameters email is required. ' + JSON.stringify(request.params));

	new Parse.Query(Parse.User).startsWith('email', email).limit(50).include('profile').find().then(function (users) {
...
Parse.Cloud.define('SearchUsersByName', function (request, response) {
	...
	if (!name) response.error('Invalid parameters name is required. ' + JSON.stringify(request.params));

	new Parse.Query('Profile').startsWith('name', name).limit(50).find().then(function (profiles) {
