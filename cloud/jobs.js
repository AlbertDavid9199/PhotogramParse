...
Parse.Cloud.job("New_Like_Notifications", function (request, status) {
	...
	matchQuery.each(function (match) {
		...
		if (match.get('action1')) likedUserIds[match.get('uid2')] = null;else likedUserIds[match.get('uid1')] = null;
	}).then(function (result) {

		channels = _.keys(likedUserIds);
		// pre-pend 'user_' to each value in the array
		channels = _.map(channels, function (e) {
			return 'user_' + e;
		});
...
function rebuildMatches(user) {
	var query1 = new Parse.Query('Match').equalTo('uid1', user.id).equalTo('state', 'M').limit(1000);
	var query2 = new Parse.Query('Match').equalTo('uid2', user.id).equalTo('state', 'M').limit(1000);
...
Parse.Cloud.job("Fix_Mutual_Matches_Missing_Profiles", function (request, status) {
	...
	var fixMutualMatch = function fixMutualMatch(match) {
		var user1, user2;
...
Parse.Cloud.job("Fix_Duplicate_Profiles", function (request, status) {
	...
	profileQuery.find().then(function (profiles) {
		...
		var byUid = _.groupBy(profiles, function (p) {
			return p.get('uid');
		});
		...
		var dupes = _.filter(byUid, function (val) {
			return val.length > 1;
		});
	...
	var fixDupes = function fixDupes(orig, dupes) {
...
Parse.Cloud.job("Log_Duplicate_Profiles", function (request, status) {
	...
	profileQuery.find().then(function (profiles) {
		...
		var mapper = function mapper(p) {
			return { id: p.id, uid: p.uid, createdAt: p.createdAt, updatedAt: p.updatedAt };
		};
