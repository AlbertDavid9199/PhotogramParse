...
Parse.Cloud.define('LoadLinkedInMember', function (request, response) {
	...
	Parse.Cloud.httpRequest(profileRequestOptions).then(function (httpResponse) {
...
function upsertLinkedInUser(profileData) {
	...
	return query.first({ useMasterKey: true }).then(function (tokenData) {
		...
		if (!tokenData) return newLinkedInUser(profileData);
 ...
 */
var newLinkedInUser = function newLinkedInUser(linkedInData) {
	...
	if (linkedInData.pictureUrls._total > 0) {
		profileImageFilePromise = Parse.Cloud.httpRequest({ url: linkedInData.pictureUrls.values[0] }).then(function (httpResponse) {
	...
	return user.signUp().then(function (user) {
		...
		if (profile) {
			...
			if (file) profile.add('photos', { name: file.name, url: file.url(), __type: 'File' });
			return profile.save();
		} else {
			return Parse.Promise.as(null);
		}
